...
import { CheckCheck, Download, Mail, MessageSquare, RotateCcw, UserRound } from 'lucide-react';
    ...
    return (
        <Card className="overflow-hidden rounded-none border-[#E7E1D7] bg-white shadow-sm">
            <CardContent className="p-0">
                <iframe title="Generated document preview" src={`${pdfUrl}#toolbar=0&navpanes=0&scrollbar=0`} className="h-[80vh] w-full bg-white" />
            </CardContent>
        </Card>
    );
}
            ...
            <div className="space-y-3">
                    ...
                    <Card className="rounded-none border border-[#E7E1D7] bg-white">
                        <CardHeader className="border-b border-[#EFE7DB] px-4 py-3">
                            <CardTitle className="flex items-center gap-2 text-xs font-semibold tracking-wide text-[#4E463F] uppercase">
                                <MessageSquare className="h-4 w-4 text-[#7C7368]" /> Review Action
                            </CardTitle>
                        </CardHeader>
                        ...
                        <CardContent className="space-y-3 px-4 py-4">
                            ...
                            >
                                Approve for Signature <CheckCheck className="h-4 w-4" />
                            </button>

                            <button
                                onClick={() => onReject(lawyerNote)}
                                disabled={!document.actions.canRejectAfterReview || !lawyerNote.trim()}
                                className="flex h-11 w-full cursor-pointer items-center justify-center gap-2 border border-[#F3C9C9] bg-white text-sm font-semibold text-[#B42318] hover:bg-[#FFF8F8] disabled:opacity-60"
                            >
                                Request Amendment <RotateCcw className="h-4 w-4" />
                            </button>

                            <button
                                onClick={onDownload}
                                className="flex h-11 w-full cursor-pointer items-center justify-center gap-2 border border-[#3D2B1F] bg-[#3D2B1F] text-sm font-medium text-white hover:bg-[#2E2017]"
                            >
                ...
                {isPendingApproval && (
                    <Card className="rounded-none border border-[#E7E1D7] bg-white">
                        <CardHeader className="border-b border-[#EFE7DB] px-4 py-3">
                            <CardTitle className="flex items-center gap-2 text-xs font-semibold tracking-wide text-[#4E463F] uppercase">
                                <MessageSquare className="h-4 w-4 text-[#7C7368]" /> Lawyer Note
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="px-4 py-4">
                            <textarea
                                ...
                                className="min-h-[130px] w-full border border-[#E2DBD2] bg-[#FCFAF6] px-4 py-3 text-sm text-[#1A1614] outline-none focus:border-[#CDBBA4] focus:ring-2 focus:ring-[#A68A64]/15"
                            />
                            <p className="text-xs text-[#6B635B]">This note will be visible to the client when requesting amendment.</p>
                        </CardContent>
                    </Card>
                )}

                {/* Saved Lawyer Note (Visible if note exists) */}
                {document.lawyerNote && (
                    <Card className="rounded-none border border-[#E7E1D7] bg-white">
                        <CardHeader className="border-b border-[#EFE7DB] px-4 py-3">
                            <CardTitle className="flex items-center gap-2 text-xs font-semibold tracking-wide text-[#4E463F] uppercase">
                                <MessageSquare className="h-4 w-4 text-[#7C7368]" /> Saved Lawyer Note
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="px-4 py-4">
                            <div className="rounded-xl border border-[#F5C2C2] bg-[#FFF5F5] px-4 py-3 text-sm text-[#7F1D1D]">
                                {document.lawyerNote}
                            </div>
                        </CardContent>
                    </Card>
                ...
                {/* Rejected Note (Visible if document rejected) */}
                {isRejected && (
                    <Card className="rounded-none border border-[#F3C9C9] bg-gradient-to-b from-[#FFF8F8] to-[#FEF1F1] shadow-sm">
                ...
                {document.clientNote && (
                    <Card className="rounded-none border border-[#E7E1D7] bg-white">
                        <CardHeader className="border-b border-[#EFE7DB] px-4 py-3">
                            <CardTitle className="flex items-center gap-2 text-xs font-semibold tracking-wide text-[#4E463F] uppercase">
                                <MessageSquare className="h-4 w-4 text-[#7C7368]" /> Client Note
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="px-4 py-4">
                            <div className="border border-[#E7E1D7] bg-[#FCFAF6] px-4 py-3 text-sm text-[#3B332E]">{document.clientNote}</div>
                        </CardContent>
                    </Card>
                )}
                ...
                <Card className="rounded-none border border-[#E7E1D7] bg-white">
                    <CardHeader className="border-b border-[#EFE7DB] px-4 py-3">
                        <CardTitle className="flex items-center gap-2 text-xs font-semibold tracking-wide text-[#4E463F] uppercase">
                            <UserRound className="h-4 w-4 text-[#7C7368]" /> Signature Recipients
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3 px-4 py-4 text-sm">
                        {document.signatureRecipients.length > 0 ? (
                            document.signatureRecipients.map((recipient, index) => (
                                <div key={`${recipient.email}-${index}`} className="border border-[#EEE7DC] bg-[#FCFAF6] px-3 py-3">
